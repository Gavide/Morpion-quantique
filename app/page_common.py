...
def ia_jouer(plateau, marques_q, ia_algo, joueur_ia, limite_N=2, trigger_sat=False):
    if ia_algo == "random":
        return ql.choisir_coup_random(plateau, marques_q, limite_N=limite_N, trigger_sat=trigger_sat)
    elif ia_algo == "grover":
        return ql.choisir_coup_ia_grover(plateau, marques_q, joueur_ia=joueur_ia,
                                         mode='heuristique_optimal', limite_N=limite_N, trigger_sat=trigger_sat)
    elif ia_algo == "qaoa":
        return ql.choisir_coup_ia_grover(plateau, marques_q, joueur_ia=joueur_ia,
                                         mode='qaoa', limite_N=limite_N, trigger_sat=trigger_sat)
    elif ia_algo == "minimax_q":
        return ql.choisir_coup_minimax_quantum(plateau, marques_q, joueur=joueur_ia,
                                               profondeur=2, limite_N=limite_N, trigger_sat=trigger_sat)
    return ql.choisir_coup_random(plateau, marques_q, limite_N=limite_N, trigger_sat=trigger_sat)