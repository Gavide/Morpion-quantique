...
def paire_valide(c1, c2, plateau, marques_q, limite_N, trigger_sat):
    """
    Vérifie si une paire (c1, c2) est jouable selon les règles de saturation.
    Même logique que _can_click côté UI, mais pour les IAs.
    """
    if plateau[c1[0]][c1[1]] != 0 or plateau[c2[0]][c2[1]] != 0:
        return False

    if trigger_sat:
        # Saturation stricte : aucune case ne dépasse N
        for c in [c1, c2]:
            poids = sum(m[1].count(c) for m in marques_q)
            clics = ([c1, c2]).count(c)
            if poids + clics > limite_N:
                return False
        return True
    else:
        # Saturation OFF : interdit seulement si les 2 cases sont identiques
        # ET que ça dépasserait N
        if c1 == c2:
            poids = sum(m[1].count(c1) for m in marques_q)
            if poids + 2 > limite_N:
                return False
        return True
...
def choisir_coup_ia_grover(plateau, marques_q, joueur_ia=-1, mode='heuristique_optimal',
                           limite_N=2, trigger_sat=False):
    cases = cases_disponibles_quantum(plateau, marques_q)
    if not cases: return None, None
    # Paires normales (c1 != c2)
    paires = [(cases[i], cases[j]) for i in range(len(cases)) for j in range(i+1, len(cases))]
    # Paires doublées (c1 == c2) : utile pour saturer une case stratégique
    for c in cases:
        poids_ia = sum(m[1].count(c) for m in marques_q if m[0] == joueur_ia)
        if poids_ia > 0:
            paires.append((c, c))
    # Filtrer les paires invalides selon les règles de saturation
    paires = [p for p in paires if paire_valide(p[0], p[1], plateau, marques_q, limite_N, trigger_sat)]
...
def minmax_quantum(plateau, marques_q, profondeur_max, joueur, limite_N=2, trigger_sat=False):
    ...
    def score(plateau, marques_q, profondeur, joueur_actuel, alpha, beta):
        ...
        if verifier_victoire(plateau, -1): return -1000
        cases = cases_disponibles_quantum(plateau, marques_q)
        if not cases or profondeur == profondeur_max:
            return heuristique_quantum(plateau, marques_q, joueur_actuel)
        paires_brutes = [(cases[i], cases[j]) for i in range(len(cases)) for j in range(i+1, len(cases))]
        paires = [p for p in paires_brutes if paire_valide(p[0], p[1], plateau, marques_q, limite_N, trigger_sat)]
        if not paires:
            return heuristique_quantum(plateau, marques_q, joueur_actuel)
        if joueur_actuel == 1:
...
def choisir_coup_minimax_quantum(plateau, marques_q, joueur=-1, profondeur=2,
    ...
    cases = cases_disponibles_quantum(plateau, marques_q)
    if not cases: return None, None
    return minmax_quantum(plateau, marques_q, profondeur, joueur, limite_N, trigger_sat)

# ═══════════════════════════════════════════════════════════
# IA RANDOM (baseline)
# ═══════════════════════════════════════════════════════════

def choisir_coup_random(plateau, marques_q, limite_N=2, trigger_sat=False):
    cases = cases_disponibles_quantum(plateau, marques_q)
    if not cases: return None, None
    paires = [(cases[i], cases[j]) for i in range(len(cases)) for j in range(i+1, len(cases))]
    for c in cases:
