...
from page_jeu_local import _can_click
...
def show():
    ...
    retour.bouton_retour(
        f"IA : <code>{ia_algo}</code> &nbsp;|&nbsp; {p1n} vs Ordinateur",
        avec_reset=True)
    ...
    if st.session_state.winner is not None:
        ...
        if active == JH:
            ...
            st.markdown(
                ...
                f"Tour : Ordinateur ({ia_algo}) — réfléchit...</h3>",
                unsafe_allow_html=True)

    # ── Grille + historique (toujours rendus) ──────────────────
    ...
    with cg:
        ...
        if st.session_state.winner is None and active == JH:
            if clicked:
                if (len(st.session_state.selection) < LIM and
                        _can_click(clicked, st.session_state.selection,
                                   st.session_state.marques_q, LIM, SAT)):
    ...
    # L'historique et le circuit sont déjà visibles avant ce point.
    if st.session_state.winner is None and active == JIA:
        with st.spinner(f"L'IA ({ia_algo}) réfléchit..."):
            c1, c2 = ia_jouer(st.session_state.plateau, st.session_state.marques_q,
                              ia_algo, JIA, limite_N=LIM, trigger_sat=SAT)
        if c1 is not None:
